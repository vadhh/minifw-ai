...
# Serialises all read-modify-write cycles so concurrent admin requests
# cannot clobber each other's policy edits.
...
def update_segment_subnets(segment_name: str, subnets: list):
    ...
    # Validate subnets before acquiring the lock (no I/O needed)
    ...
    with _policy_lock:
        ...
        if "segments" not in policy or segment_name not in policy["segments"]:
            raise ValueError(f"Segment '{segment_name}' does not exist. Create segment first.")
...
def update_features(dns_weight: int, sni_weight: int, asn_weight: int, burst_weight: int):
...
def update_enforcement(ipset_name_v4: str, ip_timeout_seconds: int, nft_table: str, nft_chain: str):
...
def update_collectors(dnsmasq_log_path: str, zeek_ssl_log_path: str, use_zeek_sni: bool):
    ...
    # Security: validate paths before acquiring lock (no I/O needed)
    ...
    for path in [dnsmasq_log_path, zeek_ssl_log_path]:
        ...
        if not is_allowed:
            raise ValueError(f"Security Error: Path '{path}' is not allowed. Must be under: {allowed_prefixes}")
...
def update_burst(dns_queries_per_minute_monitor: int, dns_queries_per_minute_block: int):
