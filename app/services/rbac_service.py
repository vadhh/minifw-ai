...
class RBACService:
    ...
    def check_permission(
        self, user: User, required_role: UserRole, action: str = "perform this action"
    ) -> bool:
        """
    ...
    def check_sector_access(
        self, user: User, target_sector: SectorType, action: str = "access this sector"
    ) -> bool:
        """
...
class AuditService:
    ...
    @staticmethod
    def log_user_action(
        db: Session, user: User, action: AuditAction, description: str, **kwargs
    ) -> AuditLog:
    ...
    def log_enforcement_action(
        ...
        return AuditService.log_action(
            ...
            severity=(
                ...
                if action == AuditAction.IP_BLOCKED
                else AuditSeverity.INFO
            ),
        )

    @staticmethod
    def get_user_activity(db: Session, user_id: int, limit: int = 100, offset: int = 0):
    ...
    @staticmethod
    def get_policy_history(db: Session, policy_id: str, limit: int = 50):
        """Get change history for a specific policy"""
        return (
            db.query(AuditLog)
            .filter(
                AuditLog.resource_type == "policy", AuditLog.resource_id == policy_id
            )
            .order_by(AuditLog.timestamp.desc())
            .limit(limit)
            .all()
    ...
    def get_enforcement_logs(
        ...
        if sector:
            ...
            query = query.filter(AuditLog.extra_data.contains(f'"sector": "{sector}"'))
    ...
    def get_security_events(
        db: Session, severity: Optional[AuditSeverity] = None, limit: int = 500
    ):
