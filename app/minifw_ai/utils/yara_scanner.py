...
class YARAScanner:
    ...
    def __init__(
        ...
        if rules_dir is None:
            rules_dir = os.getenv("MINIFW_YARA_RULES", "/opt/minifw_ai/yara_rules")
    ...
    def scan_payload(
        self, payload: bytes | str, timeout: int = 60, return_strings: bool = True
    ) -> List[YARAMatch]:
        """
        ...
        try:
            # Perform scan
            matches = self.compiled_rules.match(data=payload, timeout=timeout)
            ...
            for match in matches:
                ...
                self.scans_by_category[category] = (
                    self.scans_by_category.get(category, 0) + 1
                )

            if results:
                logger.info(
                    f"YARA scan: {len(results)} matches in {len(payload)} bytes"
                )
    ...
    def get_match_summary(self, matches: List[YARAMatch]) -> dict:
        ...
        """
        if not matches:
            return {"total_matches": 0, "categories": {}, "severities": {}, "rules": []}
        ...
        return {
            ...
            "highest_severity": (
                max(
                    severities.keys(),
                    key=lambda k: {"critical": 4, "high": 3, "medium": 2, "low": 1}.get(
                        k, 0
                    ),
                )
                if severities
    ...
    def get_stats(self) -> dict:
        ...
        return {
            ...
            "match_rate": (
                self.total_matches / self.total_scans if self.total_scans > 0 else 0.0
            ),
...
def get_yara_scanner(
    rules_dir: Optional[str] = None, force_reload: bool = False
) -> YARAScanner:
    """
