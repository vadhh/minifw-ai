...
def get_all_audit_logs_controller(
    ...
    return {"logs": logs_data, "total": total, "limit": limit, "offset": offset}


def get_audit_statistics_controller(db: Session, current_user: User, days: int = 7):
    """
    ...
    policy_changes = query.filter(
        AuditLog.action.in_(["policy_created", "policy_updated", "policy_deleted"])
    ).count()

    enforcement_actions = query.filter(
        AuditLog.action.in_(
    ...
    recent_critical = (
        db.query(AuditLog)
        .filter(AuditLog.severity == "critical", AuditLog.timestamp >= start_date)
    ...
    return {
        ...
        "by_severity": {"critical": critical, "warning": warnings, "info": info},
        "by_category": {
...
def export_audit_logs_controller(
    ...
    if current_user.role not in ["super_admin", "admin", "auditor"]:
        raise HTTPException(
            status_code=403, detail="You do not have permission to export audit logs"
        )
