...
# ============================================================
# SECTOR LOCK (read-only, factory-set)
# ============================================================

@router.get("/api/sector-lock")
def get_sector_lock_status(_: User = Depends(get_current_user)):
    """
    Get the factory-set sector lock status.

    Returns the immutable sector configuration. The sector cannot be
    changed via the Admin UI — it is stamped at package build time.
    """
    try:
