...
if __name__ == '__main__':
    ...
    if is_threat:
        # Threat patterns: high burst, many small packets, short duration
        pattern = 'bot_traffic'
        duration = random.uniform(1, 5)
        pkt_count = random.randint(50, 500)
        pkt_size_range = (60, 200)
        burst_factor = 3.0  # High burst
    else:
        # Normal patterns: varied by type
    ...
    print()
    print("By Action:")
    print(f"  Allow:   {stats['allow']:4d} ({stats['allow']/len(records)*100:5.1f}%)")
    print(f"  Monitor: {stats['monitor']:4d} ({stats['monitor']/len(records)*100:5.1f}%)")
    print(f"  Block:   {stats['block']:4d} ({stats['block']/len(records)*100:5.1f}%)")
    print()
    print("By Label:")
    print(f"  Normal (0): {stats['normal']:4d} ({stats['normal']/len(records)*100:5.1f}%)")
    print(f"  Threat (1): {stats['threats']:4d} ({stats['threats']/len(records)*100:5.1f}%)")
    print()
    ...
    if 0.20 <= threat_ratio <= 0.40:
        ...
        print(f"  Threat ratio: {threat_ratio*100:.1f}%")
        print("  Recommended: 20-40%")
    
    print()
    
    if len(records) >= 1000:
        print("✓ EXCELLENT: Sufficient data for training (1000+ flows)")
    elif len(records) >= 500:
        print("✓ GOOD: Adequate data for initial training")
    else:
        print("⚠ Need more data: Collect 500+ flows for better model")
    ...
    print("=" * 70)
    print()
    print(f"JSONL (raw): {jsonl_file}")
    print(f"CSV (labeled): {csv_file}")
    print()
    print("Files are ready for:")
    ...
    print()
    print("2. Train MLP model (STEP 1.4):")
    print(f"   python3 train_mlp.py --data {csv_file}")
    print()
    print("3. Or generate more data:")
    print(f"   python3 test_standalone_integration.py 5000")
    print()
