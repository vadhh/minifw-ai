...
class TestStreamDnsEventsJournald:
    ...
    def test_reconnect_on_unexpected_exit(self, mock_popen, mock_sleep):
        """If journalctl exits, collector yields degraded events then reconnects."""
        ...
        gen = stream_dns_events_journald()
        # Skip degraded (None, None) yields during backoff
        event = next(gen)
        while event == (None, None):
            event = next(gen)

        assert event == ("127.0.0.1", "retry.com")
        # sleep was called during backoff
