...
def _filter_events(events: List[Dict], search_value: str) -> List[Dict]:
    ...
    for event in events:
        ...
        if (
            search_lower in event.get("time", "").lower()
            or search_lower in event.get("type", "").lower()
            or search_lower in event.get("source", "").lower()
            or search_lower in event.get("status", "").lower()
        ):
...
def _sort_events(events: List[Dict], order_column: int, order_dir: str) -> List[Dict]:
    ...
    """
    # Column mapping
    column_map = {0: "time", 1: "type", 2: "source", 3: "status"}
    ...
    try:
        ...
        sorted_events = sorted(
            events, key=lambda x: x.get(sort_key, ""), reverse=reverse
        )
