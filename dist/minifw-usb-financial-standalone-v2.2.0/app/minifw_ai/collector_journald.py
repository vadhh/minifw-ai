...
_MAX_BACKOFF = 300  # max backoff delay in seconds (5 minutes)
_MAX_DEGRADED_YIELDS = 10  # yield (None, None) this many times before retrying
    ...
    while True:
        ...
        try:
            ...
            logging.warning(
                ...
                "retrying in %ds (BASELINE_PROTECTION active)",
                backoff,
            )
            ...
            logging.warning(
                "[DNS_COLLECTOR] Failed to start journalctl: %s — "
                "retrying in %ds (BASELINE_PROTECTION active)",
                e,
                backoff,
            )
        ...
        if proc is None:
            # Yield degraded events during backoff, then retry
            for _ in range(_MAX_DEGRADED_YIELDS):
                yield None, None
                time.sleep(backoff / _MAX_DEGRADED_YIELDS)
            backoff = min(backoff * 2, _MAX_BACKOFF)
            continue

        # Subprocess started successfully — reset backoff
        backoff = _RECONNECT_DELAY
        logging.info(
        ...
        # Check stderr for permission issues (log but still retry)
        try:
        ...
        # Yield degraded events during reconnect wait
        for _ in range(_MAX_DEGRADED_YIELDS):
            yield None, None
            time.sleep(backoff / _MAX_DEGRADED_YIELDS)
        backoff = min(backoff * 2, _MAX_BACKOFF)
